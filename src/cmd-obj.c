...
static enum arg_tag art_tag_lookup(const char *tag)
{
...
static bool spell_identify_unknown_available(void)
{
	...
	floor_num = scan_floor(floor_list,
			       floor_max,
...
void do_cmd_uninscribe(struct command *cmd)
{
	...
	if (cmd_get_item(cmd,
			 "item",
			 &obj,
			 "Uninscribe which item?",
			 "You have nothing you can uninscribe.",
			 obj_has_inscrip,
			 mode) != CMD_OK)
		return;
...
void do_cmd_wield(struct command *cmd)
{
	...
	/*
	 * Usually if the slot is taken we'll just replace the item in the
	 * slot, but for rings we need to ask the user which slot they
					...
					int snd)
{
	...
	if ((use != USE_CHARGE && use != USE_TIMEOUT) || check_devices(obj)) {
		...
		/*
		 * A bit of a hack to make ID work better. Check for
		 * "obvious" effects beforehand.
		 */
		if (effect->index == EF_IDENTIFY)
	...
	if (used && (use == USE_CHARGE)) {
		...
		if (object_is_carried(player, obj))
			used_obj = gear_object_for_use(obj,
						       1,
						       true,
						       &none_left);
		else
			/* Destroy an item on the floor */
			used_obj = floor_object_for_use(obj,
							1,
							true,
							&none_left);
		if (used_obj->known)
	...
	/*
	 * Hack to make Glyph of Warding work properly by pushing any objects
	 * off the Glyph square
	 */
...
void do_cmd_cast(struct command *cmd)
{
	...
	/* Get arguments */
	if (cmd_get_spell(cmd,
			  "spell",
			  &spell_index,
			  "cast",
			  obj_can_cast_from,
			  "There are no spells you can cast.",
			  spell_okay_to_cast) != CMD_OK)
		return;
	...
	/* Check for unknown objects to prevent wasted player turns. */
	if (spell_is_identify(spell_index) &&
	    !spell_identify_unknown_available()) {
		msg("You have nothing to identify.");
		return;
	}
