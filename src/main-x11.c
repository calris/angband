...
static byte x11_get_color_component(int color_idx, enum x11_col_comp col_comp)
{
	return color_table_x11[color_idx][col_comp];
}

static void x11_set_color(int color_idx) {
	/* Save new values */
	color_table_x11[color_idx][0] = angband_color_table[color_idx][0];
	color_table_x11[color_idx][1] = angband_color_table[color_idx][1];
	color_table_x11[color_idx][2] = angband_color_table[color_idx][2];
	color_table_x11[color_idx][3] = angband_color_table[color_idx][3];
...
void x11_map_colors(void)
{
	...
	for (i = 0; i < MAX_COLORS; i++) {
		if ((x11_get_color_component(i, 0) != angband_color_table[i][0]) ||
			(x11_get_color_component(i, 1) != angband_color_table[i][1]) ||
			(x11_get_color_component(i, 2) != angband_color_table[i][2]) ||
			(x11_get_color_component(i, 3) != angband_color_table[i][3])) {
			pixell pixel;

			x11_set_color(i);

			/* Create pixel */
			pixel = create_pixel(x11_get_color_component(i, 1),
					x11_get_color_component(i, 2),
					x11_get_color_component(i, 3));

			/* Change the foreground */
			x11_color_change_fg(i, pixel);
		}
	}
}
...
errr init_x11(int argc, char **argv)
{
	...
	for (i = 0; i < MAX_COLORS * BG_MAX; ++i) {
		...
		if (x11_display_is_color()) {
			...
			/* Create pixel */
			pixel = create_pixel(color_table_x11[i % MAX_COLORS][X11_COL_COMP_RED],
								 color_table_x11[i % MAX_COLORS][X11_COL_COMP_GREEN],
								 color_table_x11[i % MAX_COLORS][X11_COL_COMP_BLUE]);
			...
			switch (i / MAX_COLORS) {
				...
				case BG_SAME: {
					...
					bp = create_pixel(color_table_x11[i % MAX_COLORS][X11_COL_COMP_RED],
									  color_table_x11[i % MAX_COLORS][X11_COL_COMP_GREEN],
									  color_table_x11[i % MAX_COLORS][X11_COL_COMP_BLUE]);
				...
				case BG_DARK: {
					...
					bp = create_pixel(color_table_x11[COLOUR_SHADE][X11_COL_COMP_RED],
									  color_table_x11[COLOUR_SHADE][X11_COL_COMP_GREEN],
									  color_table_x11[COLOUR_SHADE][X11_COL_COMP_BLUE]);
			...
			/* Handle monochrome */
			x11_color_init(i, pixel, x11_display_color_bg(), CPY, 0);
		}
	}
